...
inputs_1 = checkAndRead(config['data_directory'] .. config['prior_training_outputs'])
inputs_2 = checkAndRead(config['data_directory'] .. config['prior_training_lsdf_outputs'])
...
-- Load random codes.
print('[Training] writing ' .. config['codes_file'])
testCodes = torch.randn(100, config['code'])
lib.utils.writeHDF5(config['base_directory'] .. config['codes_file'], testCodes)
...
for t = 1, iterations do
  ...
  if t%testIterations == 0 or t == 1 or t == iterations then
    ...
    local randomFile = config['base_directory'] .. t .. '_random.h5'
...
print('[Training] The model has been saved to ' .. modelFile .. '.')
print('[Training] You can run python vae_test.py config_file to evaluate the shape prior and generate OFF and BINVOX files for visualization.')
print('[Training] To train a shape completion model, please consult the documentation.')