...
if __name__ == '__main__':
    ...
    predictions_file = predictions_files[sorted(predictions_files, key=int)[-1]]#is_directory + str(iteration) + '_mean_predictions.h5'
    ...
    occ_error = np.sum(np.abs(occ_predictions - occ_targets)) / (predictions.shape[0] * predictions.shape[2] * predictions.shape[3] * predictions.shape[4])
    occ_error_thresh = np.sum(np.abs(occ_predictions_thresh - occ_targets)) / (predictions.shape[0] * predictions.shape[2] * predictions.shape[3] * predictions.shape[4])
    sdf_error = np.sum(np.abs(sdf_predictions - sdf_targets)) / (predictions.shape[0] * predictions.shape[2] * predictions.shape[3] * predictions.shape[4])
    sdf_error_thresh = np.sum(np.abs(sdf_predictions_thresh - occ_targets)) / (predictions.shape[0] * predictions.shape[2] * predictions.shape[3] * predictions.shape[4])
