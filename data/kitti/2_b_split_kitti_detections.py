...
if __name__ == '__main__':
    ...
    bounding_box_directory = common.filename(config, 'bounding_box_int_directory', '', set)
    label_directory = config['label_directory']
    label_gt_directory = config['label_gt_directory']
    bounding_box_gt_directory = common.filename(config, 'bounding_box_gt_directory', '', set)

    multiplier = config['multiplier']

    if not os.path.exists(bounding_box_directory):
        os.makedirs(bounding_box_directory)
    if not os.path.exists(bounding_box_gt_directory):
        os.makedirs(bounding_box_gt_directory)
    ...
    for i in split:
        ...
        bounding_box_file = label_gt_directory + '/%06d.txt' % i
        bounding_boxes = BoundingBox.from_kitti(bounding_box_file)
        ...
        bounding_box_file = bounding_box_gt_directory + '/%06d.txt' % i
        with open(bounding_box_file, 'w') as f:
            f.write(str(len(multiplied_bounding_boxes)) + '\n')
            for bounding_box in multiplied_bounding_boxes:
                f.write(str(bounding_box) + '\n')
