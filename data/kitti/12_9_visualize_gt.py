...
if __name__ == '__main__':
    ...
    txt_gt_directory = common.filename(config, 'velodyne_gt_txt_directory', '', set)

    txt_files = read_ordered_directory(txt_directory)
    txt_gt_files = read_ordered_directory(txt_gt_directory)
    ...
    for i in range(N):
        ...
        txt_gt_file = txt_gt_files[n]

        camera_target = initialize()
        txt_material = make_material('BRC_Material_Point_Cloud', (0.65, 0.23, 0.25), 1, True)
        txt_gt_material = make_material('BRC_Material_GT_Point_Cloud', (0.22,0.54, 0.19), 1, True)

        load_txt(txt_gt_file, 0.007, txt_gt_material, (-width*scale/2, -depth*scale/2, -height*scale/2), scale, 'xzy')
