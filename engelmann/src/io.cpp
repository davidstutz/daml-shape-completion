...
std::shared_ptr<gvl::Pointcloud> read_pointcloudPly(std::string pointcloud_path) {
...
void write_pointcloudPly(const std::shared_ptr<gvl::Pointcloud>& pointcloud,
...
std::shared_ptr<gvl::Pointcloud> read_pointcloudTxt(std::string pointcloud_path) {

  std::ifstream file(pointcloud_path.c_str());
  std::string line;
  std::stringstream ss;
  ...
  auto pointcloud(std::make_shared<gvl::Pointcloud>());
  pointcloud->points.resize(n_points);
...
void write_pointcloudTxt(const std::shared_ptr<gvl::Pointcloud>& pointcloud,
                      std::string pointcloud_path)
{
  assert(false);
}
